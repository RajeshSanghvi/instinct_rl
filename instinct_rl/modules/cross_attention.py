...
Pipeline (per frame)::
    ...
    depth ─Conv tokenizer─► N image tokens ─(+pos)─► self-attn ─► FFN ─┐ (K, V)
    proprio ─info MLP─► 1 query token ─────────────────────────────────┤ (Q)
                                                                        ▼
...
class CrossAttnFuseHeadModel(nn.Module):
    ...
    def forward(self, img: torch.Tensor, info: torch.Tensor) -> torch.Tensor:
        ...
        q = self.info_proj(info).unsqueeze(1)  # (N, 1, d_model)
        ca, _ = self.cross_attn(q, x, x, need_weights=False)  # (N, 1, d_model)
